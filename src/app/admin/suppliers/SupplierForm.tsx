...
export default function SupplierForm({ initial }: { initial: any }) {
  const [state, formAction, pending] = useActionState(upsertSupplier, null);
  const isEdit = !!initial?.id;

  return (
    <div className="container mx-auto p-6 lg:p-10 max-w-3xl">
      <div className="mb-6">
        <Link href="/admin/suppliers" className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-navy-900">
          <ArrowLeft className="h-4 w-4" /> Back to Suppliers
        </Link>
      </div>

      <div className="mb-8">
        <h1 className="text-3xl font-bold text-navy-900">{isEdit ? 'Edit Supplier' : 'New Supplier'}</h1>
      </div>
      ...
      <form action={formAction} className="rounded-md border bg-white p-6 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="contact_person" name="contact_person" defaultValue={initial?.contact_person ?? ''} />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5 md:col-span-2">
            ...
            <Input id="payment_terms" name="payment_terms" defaultValue={initial?.payment_terms ?? ''} placeholder="e.g. Net 30, Cash on delivery" />
          </div>
          <div className="space-y-1.5 md:col-span-2">
          ...
          <div className="flex items-center gap-2 md:col-span-2">
            <input type="checkbox" id="is_active" name="is_active" defaultChecked={initial?.is_active ?? true} />
        ...
        <div className="pt-4 flex justify-end gap-4 border-t">
          <Link href="/admin/suppliers"><Button variant="outline" type="button">Cancel</Button></Link>
